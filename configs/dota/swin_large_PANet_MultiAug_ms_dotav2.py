...
work_dir = 'work_dirs/swin_large_PANet_MultiAug_ms_dotav2/'

model = dict(
    type='OrientedRepPointsDetector',
    pretrained='/checkpoints_torch1.4/swin_large_patch4_window12_384_22kto1k.pth',
    backbone=dict(
        type='SwinTransformer',
        embed_dim=192,         # tiny 96    small 96       base 128      large 192
        ...
        patch_norm=True,
        out_indices=(1, 2, 3),  # strides: [4, 8, 16, 32]  channel:[192, 384, 768, 1536]
        use_checkpoint=True
    ),
    neck=
        dict(
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=data_root + 'trainval_split_1024/Train_dotav2_trainval1024ms_poly.json',
        img_prefix=data_root + 'trainval_split_1024ms/images/',
    ...
    test=dict(
        type=dataset_type,
        ann_file=data_root + 'test-dev_split/Test_datav2_test1024ms.json',
        img_prefix=data_root + 'test-dev_split_1024ms/images/',
        pipeline=test_pipeline))
